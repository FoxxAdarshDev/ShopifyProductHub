...
**Critical SKU Search Fix** (January 30, 2025): Resolved major issue where SKU search was inconsistent and only worked for a limited subset of products. Root cause was that `getProductBySku` function had a pagination limit of 10 pages (2,500 products max) while the store has 334+ products and many were beyond this limit. Updated the function to use the same comprehensive search approach as Product ID search, now searching through ALL products in the store (334 total). SKU search now works consistently for any product SKU, matching the reliability of Product ID search. Fixed fuzzy matching logic to handle exact matches, partial matches, and variant-based matches for complete SKU search coverage.
